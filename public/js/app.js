...
function _removeItem() {
  _removeItem = _asyncToGenerator( /*#__PURE__*/_babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_0___default.a.mark(function _callee(item) {
    var res;
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            _context.next = 3;
            return axios__WEBPACK_IMPORTED_MODULE_1___default.a.post('/remove-item', item);
          ...
          case 3:
            ...
            cartCounter.innerText = res.data.cart.totalQty;
            populateCart(res.data.cart);
            return _context.abrupt("return", res.data.cart);
...
function populateCart(data) {
  ...
  for (var _i = 0, _Object$values = Object.values(data.items); _i < _Object$values.length; _i++) {
    var cartItem = _Object$values[_i];
...
removeOneBtn.forEach(function (btn) {
  ...
  btn.addEventListener('click', function (e) {
    // remove from cart
    if (removeItem(item)) {
      var count = +btn.nextElementSibling.innerText;
      var newCount = count - 1;

      if (newCount > 0) {
        var lessBtnHTML = newCount < 2 ? "<span class=\"material-icons\">delete_outline</span>" : "<span class=\"material-icons\">remove</span>";
        btn.innerHTML = lessBtnHTML;
        btn.nextElementSibling.innerText = "".concat(newCount);
      } else {
...
var editAddressDataBtns = document.querySelectorAll('.editAddressDataBtn');
editAddressDataBtns.forEach(function (btn) {
  btn.addEventListener('click', function (e) {
    var _JSON$parse = JSON.parse(btn.dataset.address),
        title = _JSON$parse.title,
        locality = _JSON$parse.locality,
